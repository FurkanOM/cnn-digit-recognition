...
def iter_python_files() -> Iterator[Path]:
    ...
    Returns:
        Iterator[Path]: Python files included in the lint pass.
    """
...
def check_syntax(path: Path, source: str) -> List[str]:
    ...
    Returns:
        List[str]: Collected syntax issues.
    """
    try:
...
def check_trailing_whitespace(path: Path, source: str) -> List[str]:
    """Reject trailing whitespace and tabs that make reviews noisier.
    ...
    Returns:
        List[str]: Collected whitespace issues.
    """
    issues = []
...
def check_docstrings(path: Path, tree: ast.Module) -> List[str]:
    ...
    Returns:
        List[str]: Collected docstring issues.
    """
    issues = []
...
def check_annotations(path: Path, tree: ast.Module) -> List[str]:
    ...
    Returns:
        List[str]: Collected annotation issues.
    """
    issues = []
    relative_path = path.relative_to(ROOT)
    for node in ast.walk(tree):
        ...
        if node.returns is None:
            issues.append(
                f"{relative_path}:{node.lineno}: missing return type annotation for `{node.name}`"
            )
    return issues
